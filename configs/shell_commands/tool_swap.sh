...
filename=/home/klippy/printer_data/config/printer.cfg
...
replace_string="[include print_cores/tool$tool_number.cfg]"
...
temp_file=$(mktemp)
sed -e '/\[include print_cores\/tool[0-9]\+\.cfg\]/c'"$replace_string" "$filename" > "$temp_file"
...
then
    ...
    occurrences=$(grep -c '\[include print_cores/tool[0-9]\+\.cfg\]' "$filename")
    if [ "$occurrences" -gt 1 ]
    then
